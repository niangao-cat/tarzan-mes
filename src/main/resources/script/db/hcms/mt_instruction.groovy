...
databaseChangeLog(logicalFilePath: 'script/db/mt_instruction.groovy') {
    changeSet(author: "liyuan.lv@hand-china.com", id: "2020-05-20-mt_instruction") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_instruction_s', startValue:"1")
        }
        createTable(tableName: "mt_instruction", remarks: "仓储物流指令内容表") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "INSTRUCTION_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "主键ID ,表示唯一一条记录")  {constraints(primaryKey: true)} 
            ...
            column(name: "INSTRUCTION_TYPE", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "指令移动类型（从供应商接收，向供应商退货，发运给客户，从客户退货，工厂内部转移，组织间调拨，杂项发出，杂项接收、发出自站点、接收至站点）")  {constraints(nullable:"false")}  
            ...
            column(name: "SOURCE_ORDER_LINE_ID", type: "varchar(" + 100 * weight + ")",  remarks: "订单行id")   
            column(name: "SOURCE_ORDER_LINE_LOCATION_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "订单发运行id（oracle的采购订单发运行id）")   
            column(name: "SOURCE_ORDER_LINE_DIST_ID", type: "varchar(" + 100 * weight + ")",  remarks: "订单分配行id（oracle的分配行id）")   
            ...
            column(name: "BUSINESS_TYPE", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "业务类型")   
        ...
        addUniqueConstraint(columnNames:"INSTRUCTION_NUM,TENANT_ID",tableName:"mt_instruction",constraintName: "MT_INSTRUCTION_U1")
        addUniqueConstraint(columnNames:"IDENTIFICATION,TENANT_ID",tableName:"mt_instruction",constraintName: "MT_INSTRUCTION_U2")
    }
}