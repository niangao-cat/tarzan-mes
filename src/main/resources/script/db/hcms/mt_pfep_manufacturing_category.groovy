...
databaseChangeLog(logicalFilePath: 'script/db/mt_pfep_manufacturing_category.groovy') {
    changeSet(author: "jiangling.zheng@hand-china.com", id: "2020-06-30-mt_pfep_manufacturing_category") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_pfep_manufacturing_category_s', startValue:"1")
        }
        createTable(tableName: "mt_pfep_manufacturing_category", remarks: "物料类别生产属性") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "PFEP_MANUFACTURING_CATEGORY_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "主键ID，标识唯一一条记录")  {constraints(primaryKey: true)} 
   ...
   createIndex(tableName: "mt_pfep_manufacturing_category", indexName: "MT_PFEP_MANUFACTURING_CATG_N1") {
            column(name: "ENABLE_FLAG")
        }
   createIndex(tableName: "mt_pfep_manufacturing_category", indexName: "MT_PFEP_MANUFACTURING_CATG_N2") {
            column(name: "DEFAULT_BOM_ID")
        }
   createIndex(tableName: "mt_pfep_manufacturing_category", indexName: "MT_PFEP_MANUFACTURING_CATG_N3") {
            column(name: "DEFAULT_ROUTING_ID")
        }
   createIndex(tableName: "mt_pfep_manufacturing_category", indexName: "MT_PFEP_MANUFACTURING_CATG_N4") {
            column(name: "ISSUE_CONTROL_TYPE")
        }
   createIndex(tableName: "mt_pfep_manufacturing_category", indexName: "MT_PFEP_MANUFACTURING_CATG_N5") {
            column(name: "COMPLETE_CONTROL_TYPE")
        }
   createIndex(tableName: "mt_pfep_manufacturing_category", indexName: "MT_PFEP_MANUFACTURING_CATG_N6") {
            column(name: "ATTRITION_CONTROL_TYPE")
        }

        addUniqueConstraint(columnNames:"MATERIAL_CATEGORY_SITE_ID,ORGANIZATION_TYPE,ORGANIZATION_ID,TENANT_ID",tableName:"mt_pfep_manufacturing_category",constraintName: "MT_PFEP_MANUFACTURING_CATG_U1")
    }
}