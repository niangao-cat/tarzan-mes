...
databaseChangeLog(logicalFilePath: 'script/db/mt_material_lot_his.groovy') {
    changeSet(author: "jiangling.zheng@hand-china.com", id: "2020-06-30-mt_material_lot_his") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_material_lot_his_s', startValue:"1")
        }
        createTable(tableName: "mt_material_lot_his", remarks: "物料批历史") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "MATERIAL_LOT_HIS_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "物料批历史ID")  {constraints(primaryKey: true)} 
            ...
            column(name: "ENABLE_FLAG", type: "varchar(" + 1 * weight + ")",   defaultValue:"",   remarks: "描述该物料批的有效状态：Y：是，表示物料批当前有效N：否，表示物料批当前已经无效")  {constraints(nullable:"false")}  
            column(name: "QUALITY_STATUS", type: "varchar(" + 255 * weight + ")",   defaultValue:"",   remarks: "物料批所标识实物的质量状态物料批所标识实物的质量状态：OK：合格，表示实物目前质量合格，允许正常生产使用或者发货NG：不合格，表示实物目前质量不合格，可能需要退货或返修重工PENDING：待定，表示实物目前质量待定，可能需要进行质量检验才能进行使用：")  {constraints(nullable:"false")}  
            ...
            column(name: "TRX_PRIMARY_QTY", type: "decimal(20,6)",  remarks: "事务数量-主单位用量")   
            column(name: "TRX_SECONDARY_QTY", type: "decimal(20,6)",  remarks: "事务数量-辅助单位用量")   
        ...
        addUniqueConstraint(columnNames:"MATERIAL_LOT_ID,EVENT_ID,TENANT_ID",tableName:"mt_material_lot_his",constraintName: "MT_MATERIAL_LOT_HIS_U1")
    }
}