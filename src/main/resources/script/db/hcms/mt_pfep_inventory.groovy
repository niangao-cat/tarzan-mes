...
databaseChangeLog(logicalFilePath: 'script/db/mt_pfep_inventory.groovy') {
    changeSet(author: "jiangling.zheng@hand-china.com", id: "2020-06-30-mt_pfep_inventory") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_pfep_inventory_s', startValue:"1")
        }
        createTable(tableName: "mt_pfep_inventory", remarks: "物料存储属性") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "PFEP_INVENTORY_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "主键ID，表示唯一一条记录")  {constraints(primaryKey: true)} 
   ...
   createIndex(tableName: "mt_pfep_inventory", indexName: "MT_PFEP_INVENTORY_N1") {
            column(name: "ENABLE_FLAG")
        }
   createIndex(tableName: "mt_pfep_inventory", indexName: "MT_PFEP_INVENTORY_N2") {
            column(name: "IDENTIFY_ID")
        }
   createIndex(tableName: "mt_pfep_inventory", indexName: "MT_PFEP_INVENTORY_N3") {
            column(name: "IDENTIFY_TYPE")
        }
   createIndex(tableName: "mt_pfep_inventory", indexName: "MT_PFEP_INVENTORY_N4") {
            column(name: "PACKAGE_SIZE_UOM_ID")
        }
   createIndex(tableName: "mt_pfep_inventory", indexName: "MT_PFEP_INVENTORY_N5") {
            column(name: "WEIGHT_UOM_ID")
        }
   createIndex(tableName: "mt_pfep_inventory", indexName: "MT_PFEP_INVENTORY_N6") {
            column(name: "ISSUED_LOCATOR_ID")
        }
   createIndex(tableName: "mt_pfep_inventory", indexName: "MT_PFEP_INVENTORY_N7") {
            column(name: "COMPLETION_LOCATOR_ID")
        }
   createIndex(tableName: "mt_pfep_inventory", indexName: "MT_PFEP_INVENTORY_N8") {
            column(name: "STOCK_LOCATOR_ID")
        }

        addUniqueConstraint(columnNames:"MATERIAL_SITE_ID,ORGANIZATION_TYPE,ORGANIZATION_ID,TENANT_ID",tableName:"mt_pfep_inventory",constraintName: "MT_PFEP_INVENTORY_U1")
    }
}