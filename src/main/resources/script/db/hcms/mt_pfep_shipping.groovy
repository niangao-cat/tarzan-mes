...
databaseChangeLog(logicalFilePath: 'script/db/mt_pfep_shipping.groovy') {
    changeSet(author: "jiangling.zheng@hand-china.com", id: "2020-06-30-mt_pfep_shipping") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_pfep_shipping_s', startValue:"1")
        }
        createTable(tableName: "mt_pfep_shipping", remarks: "物料发运属性") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "PFEP_SHIPPING_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "主键ID，标识唯一一条记录")  {constraints(primaryKey: true)} 
   ...
   createIndex(tableName: "mt_pfep_shipping", indexName: "MT_PFEP_SHIPPING_N1") {
            column(name: "ENABLE_FLAG")
        }
   createIndex(tableName: "mt_pfep_shipping", indexName: "MT_PFEP_SHIPPING_N2") {
            column(name: "PACKAGE_LIST_ID")
        }
   createIndex(tableName: "mt_pfep_shipping", indexName: "MT_PFEP_SHIPPING_N3") {
            column(name: "ATTACHMENT_LIST_ID")
        }
   createIndex(tableName: "mt_pfep_shipping", indexName: "MT_PFEP_SHIPPING_N4") {
            column(name: "SHIPPING_CALENDAR_ID")
        }
   createIndex(tableName: "mt_pfep_shipping", indexName: "MT_PFEP_SHIPPING_N5") {
            column(name: "PACKAGE_IDENTIFY_ID")
        }
   createIndex(tableName: "mt_pfep_shipping", indexName: "MT_PFEP_SHIPPING_N6") {
            column(name: "PRODUCT_IDENTIFY_ID")
        }

        addUniqueConstraint(columnNames:"MATERIAL_SITE_ID,ORGANIZATION_TYPE,ORGANIZATION_ID,CUSTOMER_ID,CUSTOMER_SITE_ID,TENANT_ID",tableName:"mt_pfep_shipping",constraintName: "MT_PFEP_SHIPPING_U1")
    }
}