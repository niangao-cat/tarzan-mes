...
databaseChangeLog(logicalFilePath: 'script/db/mt_pfep_purchase_category.groovy') {
    changeSet(author: "jiangling.zheng@hand-china.com", id: "2020-06-30-mt_pfep_purchase_category") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_pfep_purchase_category_s', startValue:"1")
        }
        createTable(tableName: "mt_pfep_purchase_category", remarks: "物料类别采购属性") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "PFEP_PURCHASE_CATEGORY_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "主键ID，表示唯一一条记录")  {constraints(primaryKey: true)} 
   ...
   createIndex(tableName: "mt_pfep_purchase_category", indexName: "MT_PFEP_PURCHASE_CATEGORY_N1") {
            column(name: "ENABLE_FLAG")
        }
   createIndex(tableName: "mt_pfep_purchase_category", indexName: "MT_PFEP_PURCHASE_CATEGORY_N2") {
            column(name: "RECEIVE_LOCATOR_ID")
        }

        addUniqueConstraint(columnNames:"MATERIAL_CATEGORY_SITE_ID,ORGANIZATION_TYPE,ORGANIZATION_ID,TENANT_ID",tableName:"mt_pfep_purchase_category",constraintName: "MT_PFEP_PURCHASE_CATEGORY_U1")
    }
}