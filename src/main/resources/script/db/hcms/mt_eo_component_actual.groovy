...
databaseChangeLog(logicalFilePath: 'script/db/mt_eo_component_actual.groovy') {
    changeSet(author: "liyuan.lv@hand-china.com", id: "2020-05-21-mt_eo_component_actual") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_eo_component_actual_s', startValue:"1")
        }
        createTable(tableName: "mt_eo_component_actual", remarks: "执行作业组件装配实绩，记录执行作业物料和组件实际装配情况") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "EO_COMPONENT_ACTUAL_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "执行作业组件实绩ID")  {constraints(primaryKey: true)} 
        ...
        addUniqueConstraint(columnNames:"MATERIAL_ID,COMPONENT_TYPE,EO_ID,ROUTER_STEP_ID,BOM_ID,OPERATION_ID,TENANT_ID,BOM_COMPONENT_ID",tableName:"mt_eo_component_actual",constraintName: "MT_EO_COMPONENT_ACTUAL_U1")
    }
}