...
databaseChangeLog(logicalFilePath: 'script/db/mt_pfep_schedule.groovy') {
    changeSet(author: "jiangling.zheng@hand-china.com", id: "2020-06-30-mt_pfep_schedule") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_pfep_schedule_s', startValue:"1")
        }
        createTable(tableName: "mt_pfep_schedule", remarks: "物料计划属性") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "PFEP_SCHEDULE_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "主键，标识唯一一条记录")  {constraints(primaryKey: true)} 
            column(name: "MATERIAL_SITE_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "物料站点主键，标识唯一物料站点对应关系，限定为计划站点")  {constraints(nullable:"false")}  
   ...
   createIndex(tableName: "mt_pfep_schedule", indexName: "MT_PFEP_SCHEDULE_N1") {
            column(name: "ENABLE_FLAG")
        }

        addUniqueConstraint(columnNames:"MATERIAL_SITE_ID,ORGANIZATION_TYPE,ORGANIZATION_ID,TENANT_ID",tableName:"mt_pfep_schedule",constraintName: "MT_PFEP_SCHEDULE_U1")
    }
}