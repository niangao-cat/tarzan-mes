...
databaseChangeLog(logicalFilePath: 'script/db/mt_container.groovy') {
    changeSet(author: "jiangling.zheng@hand-china.com", id: "2020-06-30-mt_container") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_container_s', startValue:"1")
        }
        createTable(tableName: "mt_container", remarks: "容器，一个具体的容器并记录容器的业务属性，包括容器装载实物所有者、预留对象、位置状态等，提供执行作业、物料批、容器的装载结构") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "CONTAINER_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "作为容器唯一标识，用于其他数据结构引用该容器")  {constraints(primaryKey: true)} 
            ...
            column(name: "OWNER_TYPE", type: "varchar(" + 255 * weight + ")",   defaultValue:"",   remarks: "描述容器下表示实物的所有权属于客户还是供应商，内容包括：C：客户，表示该容器表示的实物属于客户S：供应商，表示该容器表示的实物属于供应商；空：表示容器表示的实物属于厂内自有")   
        ...
        addUniqueConstraint(columnNames:"CONTAINER_CODE,TENANT_ID",tableName:"mt_container",constraintName: "MT_CONTAINER_U1")
        addUniqueConstraint(columnNames:"IDENTIFICATION,TENANT_ID",tableName:"mt_container",constraintName: "MT_CONTAINER_U2")
    }
}