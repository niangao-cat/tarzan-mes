...
databaseChangeLog(logicalFilePath: 'script/db/mt_pfep_purchase_supplier_catg.groovy') {
    changeSet(author: "jiangling.zheng@hand-china.com", id: "2020-06-30-mt_pfep_purchase_supplier_catg") {
        def weight = 1
        ...
        if(helper.dbType().isSupportSequence()){
            createSequence(sequenceName: 'mt_pfep_purchase_supplier_catg_s', startValue:"1")
        }
        createTable(tableName: "mt_pfep_purchase_supplier_catg", remarks: "物料类别供应商采购属性") {
            column(name: "TENANT_ID", type: "bigint(20)",   defaultValue:"0",   remarks: "租户ID")  {constraints(nullable:"false")}  
            column(name: "PFEP_PURCHASE_SUPPLIER_CATG_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "主键ID，表示唯一一条记录")  {constraints(primaryKey: true)} 
            column(name: "MATERIAL_CATEGORY_SITE_ID", type: "varchar(" + 100 * weight + ")",   defaultValue:"",   remarks: "物料类别站点主键，标识唯一物料类别站点分配数据，限定为采购站点")  {constraints(nullable:"false")}  
   ...
   createIndex(tableName: "mt_pfep_purchase_supplier_catg", indexName: "MT_PFEP_PURCHASE_SUPPL_CATG_N1") {
            column(name: "ENABLE_FLAG")
        }
   createIndex(tableName: "mt_pfep_purchase_supplier_catg", indexName: "MT_PFEP_PURCHASE_SUPPL_CATG_N2") {
            column(name: "SUPPLIER_ID")
        }
   createIndex(tableName: "mt_pfep_purchase_supplier_catg", indexName: "MT_PFEP_PURCHASE_SUPPL_CATG_N3") {
            column(name: "SUPPLIER_SITE_ID")
        }
   createIndex(tableName: "mt_pfep_purchase_supplier_catg", indexName: "MT_PFEP_PURCHASE_SUPPL_CATG_N4") {
            column(name: "RECEIVE_LOCATOR_ID")
        }
   createIndex(tableName: "mt_pfep_purchase_supplier_catg", indexName: "MT_PFEP_PURCHASE_SUPPL_CATG_N5") {
            column(name: "SUPPLIER_CALENDAR_ID")
        }

        addUniqueConstraint(columnNames:"MATERIAL_CATEGORY_SITE_ID,ORGANIZATION_TYPE,ORGANIZATION_ID,SUPPLIER_ID,SUPPLIER_SITE_ID,TENANT_ID",tableName:"mt_pfep_purchase_supplier_catg",constraintName: "MT_PFEP_PURCHASE_SUPPL_CATG_U1")
    }
}