 ...
 */
@RestController("mtInvItemIfaceController.v1")
@RequestMapping("/v1/{organizationId}/mt-inv-item-iface")
@Api(tags = "MtInvItemIface")
public class MtInvItemIfaceController extends BaseController {

    @Autowired
    private MtInvItemIfaceRepository repository;

    @ApiOperation("materialInterfaceImport")
    @PostMapping(value = {"/import"}, produces = "application/json;charset=UTF-8")
    @Permission(level = ResourceLevel.ORGANIZATION)
    public ResponseData<Void> materialInterfaceImport(@PathVariable("organizationId") Long tenantId) {
