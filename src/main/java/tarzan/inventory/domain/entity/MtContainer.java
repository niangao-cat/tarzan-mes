...
/**
 * 容器，一个具体的容器并记录容器的业务属性，包括容器装载实物所有者、预留对象、位置状态等，提供执行作业、物料批、容器的装载结构
 *
 * @author xinlin.zhao@hand-china.com 2019-07-30 17:04:32
 */
@ApiModel("容器，一个具体的容器并记录容器的业务属性，包括容器装载实物所有者、预留对象、位置状态等，提供执行作业、物料批、容器的装载结构")

@ModifyAudit

@Table(name = "mt_container")
@CustomPrimary
public class MtContainer extends AuditDomain implements Serializable {
    ...
    private static final long serialVersionUID = 5482166191906942620L;
    ...
    /**
     * @return 描述容器下表示实物的所有权属于客户还是供应商，内容包括：C：客户，表示该容器表示的实物属于客户S：供应商，表示该容器表示的实物属于供应商；空：表示容器表示的实物属于厂内自有
     */
