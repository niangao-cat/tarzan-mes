...
@Table(name = "mt_eo_router_actual")
@CustomPrimary
public class MtEoRouterActual extends AuditDomain implements Serializable {
    ...
    private static final long serialVersionUID = -2717031513310180407L;
    ...
    @ApiModelProperty(value = "状态（值集：未开始STANDBY、运行中（排队）RUNNING、部分完成PARTLY_COMPLETED、已完成COMPLETED）", required = true)
    @NotBlank
    @Where
    ...
    @ApiModelProperty(value = "此工艺路线已完成数量", required = true)
    @NotNull
    @Where
    ...
    /**
     * @return 状态（值集：未开始STANDBY、运行中（排队）RUNNING、部分完成PARTLY_COMPLETED、已完成COMPLETED）
     */
