 ...
 */
@ImportValidators({@ImportValidator(templateCode = "QMS.PQC_MATERIAL_INSP_SCHEME")})
public class QmsImportPqcMaterialInspSchemeValidator extends ValidatorHandler {
    @Autowired
    ...
    public boolean validate(String data) {
        ...
        if (data != null && !"".equals(data)) {
            QmsImportPqcMaterialInspSchemeVO importVO = null;
            try {
                importVO = objectMapper.readValue(data, QmsImportPqcMaterialInspSchemeVO.class);
            } catch (IOException e) {
                // 失败
                return false;
            }
     ...
     * @return
     */
    private boolean checkQmsMaterialInspSchemeVO(Long tenantId, QmsImportPqcMaterialInspSchemeVO importVO) {
        // 校验字段非空
        ...
        if (StringUtils.isNotBlank(materialCode)) {
            ...
            if (StringUtils.isNotBlank(materialVersion)) {
                boolean materialVersionIsExist = checkMaterialVersion(tenantId, importVO.getSiteId(), importVO.getMaterialId(), materialVersion);
                if (!materialVersionIsExist) {
                    return false;
                }
            }
        }
        ...
        // 校验新增情况下数据收集组和数据项唯一性
        QmsPqcInspectionContent qmsPqcInspectionContent = checkQmsPqcInspectionContent(tenantId, importVO);
        if (StringUtils.equals(importVO.getOptFlag(), "INCREASE") && Objects.nonNull(qmsPqcInspectionContent)) {
            return false;
        }
        return true;
    }

    private QmsPqcInspectionScheme checkQmsPqcInspectionScheme(Long tenantId, QmsImportPqcMaterialInspSchemeVO importVO) {
        ...
        qmsPqcInspectionScheme = qmsPqcInspectionSchemeRepository.selectOne(qmsPqcInspectionScheme);
        if (Objects.nonNull(qmsPqcInspectionScheme)) {
            // MATERIAL_INSP_SCHEME_002 --> 检测到重复数据,请检查
            getContext().addErrorMsg(mtErrorMessageRepository.getErrorMessageWithModule(tenantId,
                    "MATERIAL_INSP_SCHEME_002", "QMS"));
        }
        return qmsPqcInspectionScheme;
    }

    private QmsPqcInspectionContent checkQmsPqcInspectionContent(Long tenantId, QmsImportPqcMaterialInspSchemeVO importVO) {
        QmsPqcInspectionScheme qmsPqcInspectionScheme = new QmsPqcInspectionScheme();
        qmsPqcInspectionScheme.setTenantId(tenantId);
        qmsPqcInspectionScheme.setSiteId(importVO.getSiteId());
        qmsPqcInspectionScheme.setMaterialId(importVO.getMaterialId());
        ...
        if (Objects.nonNull(qmsPqcInspectionScheme)) {
            // 计划不为空 新增情况下 内容存在则报错
            if (StringUtils.equals(importVO.getOptFlag(), "INCREASE")) {
                QmsPqcInspectionContent qmsPqcInspectionContent = new QmsPqcInspectionContent();
                qmsPqcInspectionContent.setTenantId(tenantId);
                qmsPqcInspectionContent.setSchemeId(qmsPqcInspectionScheme.getInspectionSchemeId());
                ...
                if (Objects.nonNull(qmsPqcInspectionContent)) {
                    getContext().addErrorMsg(mtErrorMessageRepository.getErrorMessageWithModule(tenantId,
                            "HME_EXCEL_IMPORT_025", "HME", importVO.getTagGroupCode(), importVO.getTagCode()));
                }
                return qmsPqcInspectionContent;
            }
        }
        return null;
    }
     ...
     */
    private boolean checkMaterialVersion(Long tenantId, String siteId, String materialId, String materialVersion) {
        boolean flag = true;
     ...
     * @return
     */
    private boolean checkInfoNotBlank(Long tenantId, QmsImportPqcMaterialInspSchemeVO importVO) {
        boolean flag = true;
        ...
        map.put(importVO.getFrequency(), "频率");
        map.put(importVO.getOptFlag(), "标识(新增&更新)");
        ...
        if(StringUtils.equals(importVO.getInspectionType(), "NORMAL") && StringUtils.isBlank(importVO.getProcessCode())){
            getContext().addErrorMsg(mtErrorMessageRepository.getErrorMessageWithModule(tenantId,
                    "WMS_QR_CODE_0003", "WMS", "工序"));
            flag = false;
        }
