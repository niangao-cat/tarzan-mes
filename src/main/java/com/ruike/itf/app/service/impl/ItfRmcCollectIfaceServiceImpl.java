...
@Service
@Slf4j
public class ItfRmcCollectIfaceServiceImpl extends BaseServiceImpl<ItfRmcCollectIface> implements ItfRmcCollectIfaceService {

    private final ItfRmcCollectIfaceRepository itfRmcCollectIfaceRepository;
    @Autowired
    ...
    @Autowired
    private ItfRmcCollectIfaceMapper itfRmcCollectIfaceMapper;
    ...
    @Autowired
    public ItfRmcCollectIfaceServiceImpl(ItfRmcCollectIfaceRepository itfRmcCollectIfaceRepository) {
        this.itfRmcCollectIfaceRepository = itfRmcCollectIfaceRepository;
    }
     ...
     */
    private boolean validate(Long tenantId, List<ItfRmcCollectIface> itfList) {
        boolean validFlag = true;
        // 反射得到字段map
        Map<String, Field> fieldMap = Arrays.stream(ItfRmcCollectIface.class.getDeclaredFields()).collect(Collectors.toMap(Field::getName, rec -> rec, (key1, keys2) -> key1));
        for (ItfRmcCollectIface itf : itfList) {
            ...
            if (StringUtils.isNotBlank(processMessage)) {
                ...
                itfRmcCollectIfaceRepository.updateByPrimaryKeySelective(itf);
                validFlag = false;
            }
        }
        return validFlag;
    ...
    @Override
    public List<DataCollectReturnDTO> invoke(Long tenantId, List<RmcCollectItfDTO> collectList) {
        log.info("ItfRmcCollectIfaceServiceImpl.invoke.start");
        if (CollectionUtils.isEmpty(collectList)) {
            return new ArrayList<>();
        }
        // 插入接口表
        List<ItfRmcCollectIface> list = new ArrayList<>();
        BeanCopier copier = BeanCopier.create(RmcCollectItfDTO.class, ItfRmcCollectIface.class, false);
        Date nowDate = new Date();
        List<String> ifaceIdList = customSequence.getNextKeys("itf_rmc_collect_iface_s", collectList.size());
        int ifaceIdIndex = 0;
        for (RmcCollectItfDTO data : collectList) {
            ItfRmcCollectIface itf = new ItfRmcCollectIface();
        ...
        log.info("ItfRmcCollectIfaceServiceImpl.insterIface.start");
        myBatchInsert(list);
        log.info("ItfRmcCollectIfaceServiceImpl.insterIface.end");
        // 验证接口表
        log.info("ItfRmcCollectIfaceServiceImpl.check.start");
        boolean validFlag = this.validate(tenantId, list);
        log.info("ItfRmcCollectIfaceServiceImpl.check.end");
        ...
        if (validFlag) {
            ...
            if (ObjectUtil.isEmpty(hmeEquipmentFirst)) {
                for (ItfRmcCollectIface data : list) {
            ...
            log.info("ItfRmcCollectIfaceServiceImpl.OperationIds :"+OperationIds);
            ...
            log.info("ItfRmcCollectIfaceServiceImpl.materialLot :"+materialLot);
...
//            }
            ...
            log.info("ItfRmcCollectIfaceServiceImpl.REWORK_FLAG :"+mtExtendAttrVOList);
            ...
            List<HmeEoJobSn> hmeEoJobSns = hmeEoJobSnRepository.selectByCondition(Condition.builder(HmeEoJobSn.class)
                    ...
                    .build());
            log.info("ItfRmcCollectIfaceServiceImpl.hmeEoJobSns :"+hmeEoJobSns);
            ...
            log.info("ItfRmcCollectIfaceServiceImpl.dataType"+apCollectItfDTO1s);
            log.info("ItfRmcCollectIfaceServiceImpl.select.end");
            ...
            for (ItfRmcCollectIface data : list) {
                for (ApCollectItfDTO1 temp :
                    ...
                    if (!org.apache.commons.lang.StringUtils.isBlank(valueField)) {
     ...
     **/
    private String getResult(ApCollectItfDTO1 temp, ItfRmcCollectIface data) {
        if (!org.apache.commons.lang.StringUtils.isBlank(temp.getLimitCond1())) {
            String limit1 = getFieldValueByFieldName(temp.getLimitCond1(), data);
            if (!limit1.equals(temp.getCond1Value())) {
                return null;
            }
        }
        if (!org.apache.commons.lang.StringUtils.isBlank(temp.getLimitCond2())) {
     ...
     *@return void
     **/
    public void myBatchInsert(List<ItfRmcCollectIface> insertList) {
        if (org.apache.commons.collections4.CollectionUtils.isNotEmpty(insertList)) {
            List<List<ItfRmcCollectIface>> splitSqlList = InterfaceUtils.splitSqlList(insertList, SQL_ITEM_COUNT_LIMIT);
            for (List<ItfRmcCollectIface> domains : splitSqlList) {
                itfRmcCollectIfaceMapper.insertIface(domains);
            }
        }
    }
