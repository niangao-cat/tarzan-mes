 ...
 */
@ImportValidators({@ImportValidator(templateCode = "HME.MATERIAL_LOT_TWO")})
public class HmeImportMaterialLotTwoValidator extends ValidatorHandler {
    ...
    private Boolean checkHmeImportMaterialLotVO(Long tenantId, HmeImportMaterialLotVO importVO) {
        ...
        // 校验条码有效性和在制品
        if (StringUtils.isNotBlank(importVO.getMaterialLotCode())) {
            ...
            if (mtMaterialLot != null) {
                ...
                if (StringUtils.equals(mfFlag, HmeConstants.ConstantValue.YES)) {
                    getContext().addErrorMsg(mtErrorMessageRepository.getErrorMessageWithModule(tenantId,
                            "HME_MATERIAL_LOT_006", "HME", mtMaterialLot.getMaterialLotCode()));
                    return false;
                }
                // 校验有效性
                if (HmeConstants.ConstantValue.YES.equals(mtMaterialLot.getEnableFlag())) {
                    getContext().addErrorMsg(mtErrorMessageRepository.getErrorMessageWithModule(tenantId,
                            "HME_MATERIAL_LOT_005", "HME", mtMaterialLot.getMaterialLotCode()));
                    return false;
                }
            }
        }
