 ...
 * 物料存储属性导入模板(模板三)
 *
 * @author sanfeng.zhang@hand-china.com 2021/2/3 10:45
 */
@ImportValidators({@ImportValidator(templateCode = "HME.MATERIAL_LOT_THREE")})
public class HmeImportMaterialLotThreeValidator extends ValidatorHandler {
    ...
    private Boolean checkHmeImportMaterialLotVO(Long tenantId, HmeImportMaterialLotVO importVO) {
        ...
        if (StringUtils.isBlank(importVO.getMaterialLotCode())) {
            // MT_ASSEMBLE_0004 --> ${1}不存在.${2}
            getContext().addErrorMsg(mtErrorMessageRepository.getErrorMessageWithModule(tenantId,
                    "MT_ASSEMBLE_0004", "ASSEMBLE", "条码号", importVO.getMaterialLotCode()));
            return false;
        }
        MtMaterialLot mtMaterialLot = getMtMaterialLot(tenantId, importVO.getMaterialLotCode());
        if (mtMaterialLot != null) {
            getContext().addErrorMsg(mtErrorMessageRepository.getErrorMessageWithModule(tenantId,
                    "HME_MATERIAL_LOT_0001", "HME", importVO.getMaterialLotCode()));
            return false;
        }
        // 数值要等于0
        if (StringUtils.isNotBlank(importVO.getPrimaryUomQty())) {
            if (Double.valueOf(importVO.getPrimaryUomQty()).compareTo(0D) != 0) {
                getContext().addErrorMsg(mtErrorMessageRepository.getErrorMessageWithModule(tenantId,
                        "HME_MATERIAL_LOT_007", "HME", importVO.getMaterialLotCode()));
                return false;
            }
        }
